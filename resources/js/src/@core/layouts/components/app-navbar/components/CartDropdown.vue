<template>
  ...
  >
    ...
    >
      ...
      >
        <template #aside>
          <b-img v-if="item.product.files.length>0"
        ...
        <feather-icon
          ...
          @click.stop="removeItemFromCart(item.product.id)"
        />
        ...
        <div class="cart-item-qty ml-1">
          <b-form-spinbutton
            v-model="item.qty"
            @change="ChangeQtyProudct(item.product.id,item.qty)"
            min="1"
            size="sm"
          />
        </div>
    ...
    >
      <div class="d-flex justify-content-between mb-1">
        ...
        <h6 class="text-primary font-weight-bolder mb-0">
          SAR {{Math.round(totalAmount * 100) / 100 }}
        </h6>
      </div>
      <b-button
...
export default {
  ...
  computed: {
    totalAmount() {
      let total = 0
      this.items.forEach(i => { total += Number (i.product.price)*i.qty })
  ...
  methods: {
    ...
    ChangeQtyProudct(product_id,qty){
      console.log(product_id,qty)
      this.$store.dispatch('app-ecommerce/changeProductInCart',{productId:product_id,qty:qty})
    },
