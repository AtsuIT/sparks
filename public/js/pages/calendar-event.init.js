...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function (info) {
            ...
            var select = document.querySelector('#order_id');
            select.value = selectedEvent.extendedProps.order;  
    ...
    formEvent.addEventListener('submit', function (ev) {
        ...
        if (forms[0].checkValidity() === false) {
            ...
            if (selectedEvent) {
                ...
                var dEdit = selectedEvent.start;
                var curr_date_edit = dEdit.getDate();
                var curr_month_edit = dEdit.getMonth() + 1; //Months are zero based
                var curr_year_edit = dEdit.getFullYear();
                var dateFormatEdit = curr_year_edit+"-"+curr_month_edit+ "-"+curr_date_edit;
                var editEvent = {
                    title: updatedTitle,
                    start: dateFormatEdit,
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: "/update-events/"+selectedEvent.id,
                    data: editEvent,
                    success: function (response) {
                        var defaultEvents = [];
                        for(let i=0;i<=response.data.length;i++)
                        {
                            defaultEvents = response.data;
                        }
                        var calendar = new FullCalendar.Calendar(calendarEl, {
                            timeZone: 'local',
                            editable: true,
                ...
                var d = newEventData.date;
                var curr_date = d.getDate();
                var curr_month = d.getMonth() + 1; //Months are zero based
                var curr_year = d.getFullYear();
                var dateFormat = curr_year+"-"+curr_month+ "-"+curr_date;
                var newEvent = {
                    title: updatedTitle,
                    start: dateFormat,
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        type: "DELETE",
                        url: "/events-destroy/"+selectedEvent.id,
                        success: function (response) {
